...
from utils.hierarchical_processor import (
    HierarchicalContentProcessor,
    create_sections_from_text,
)
...
def test_split_into_sections():
    ...
    sections = processor.split_into_sections(
        text, min_section_size=10, max_section_size=1000
    )
    assert len(sections) >= 2
...
"""
    ...
    assert any(
        title and ("Section One" in title or "Section Two" in title)
