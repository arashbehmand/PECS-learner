...
from utils.models import Base, Project, Section, Flashcard
...
class DatabaseRepository:
    ...
    def update_project(self, project_id: int, **kwargs) -> bool:
        ...
        try:
            ...
            project.updated_at = datetime.now(UTC_TZ)
            session.commit()
            return True
        except SQLAlchemyError as e:
            session.rollback()
    ...
    # Section operations
    def create_section(self, project_id: int, content: str, title: str = None, order_index: int = 0) -> Optional[Section]:
    ...
    def update_section_pecs_data(self, section_id: int, phase: str, data: Dict[str, Any]) -> bool:
    ...
    def mark_section_completed(self, section_id: int, completed: bool = True) -> bool:
        ...
        try:
            ...
            if completed:
                section.completed_at = datetime.now(UTC_TZ)
            else:
    ...
    def create_flashcard(self, project_id: int, question: str, answer: str, section_id: Optional[int] = None) -> Optional[Flashcard]:
        ...
        try:
            flashcard = Flashcard(
                ...
                question=question,
                answer=answer,
                next_review=datetime.now(UTC_TZ)  # Start with immediate review
            )
    ...
    def get_flashcards_by_project(self, project_id: int, section_id: Optional[int] = None) -> List[Flashcard]:
    ...
    def update_flashcard_review(self, flashcard_id: int, knew_it: bool) -> bool:
        ...
        try:
            ...
            flashcard.last_reviewed = datetime.now(UTC_TZ)
            ...
            flashcard.next_review = datetime.now(UTC_TZ) + timedelta(days=flashcard.interval_days)
