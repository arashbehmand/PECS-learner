...
def build_learning_context(
    ...
    context['critical_thinking'] = challenge_data.get('critical_questions') or challenge_data.get('critical_thinking')
    ...
    logger.debug(f"Built context: {len(context['flashcards'])} flashcards, "
...
def format_context_for_flashcards(context: Dict[str, Any]) -> str:
    ...
    parts.append("=" * 60)
    parts.append(context['content'][:1500] + ("..." if len(context['content']) > 1500 else ""))
    parts.append("")
    ...
    if context.get('flashcards'):
        ...
        parts.append("")
        parts.append("⚠️  Generate DIFFERENT flashcards covering new aspects not in the list above.")
        parts.append("")
...
def format_context_for_completion(context: Dict[str, Any]) -> str:
    ...
    parts.append("LEARNING MATERIAL")
    parts.append("=" * 60)
    parts.append(context['content'][:2000] + ("..." if len(context['content']) > 2000 else ""))
    parts.append("")
