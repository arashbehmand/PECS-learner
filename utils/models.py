...
"""
from sqlalchemy import Column, Integer, String, Text, ForeignKey, JSON, Boolean, DateTime, Float, Index
...
class Project(Base):
    ...
    updated_at = Column(DateTime, default=lambda: datetime.now(UTC_TZ), onupdate=lambda: datetime.now(UTC_TZ))
...
class Section(Base):
    ...
    title = Column(String(300), nullable=True)  # Optional title like "Chapter 1: Introduction"
    ...
    flashcards = relationship("Flashcard", back_populates="section", cascade="all, delete-orphan")
...
class Flashcard(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    project_id = Column(Integer, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False)
    section_id = Column(Integer, ForeignKey("sections.id", ondelete="CASCADE"), nullable=True)  # Optional: can be global
