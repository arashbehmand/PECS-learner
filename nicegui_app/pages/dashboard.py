...
class DashboardPage:
    ...
    def render(self):
        ...
        with ui.row().classes(
            ...
            ui.label("P.E.C.S. Learning System").classes(
                "text-2xl md:text-3xl font-bold"
            )
            ui.button(
                "New Project", icon="add", on_click=self._show_create_project_dialog
            ).classes("bg-blue-500")
    ...
    def _render_empty_state(self):
        ...
        with ui.column().classes(
            "items-center justify-center mt-12 px-4 max-w-2xl mx-auto"
        ):
            ui.icon("school").classes("text-6xl text-gray-400 mb-4")
            ui.label("Welcome to P.E.C.S. Learning System!").classes(
                "text-2xl font-bold text-gray-700 text-center"
            )
            ui.label("You don't have any projects yet.").classes(
                "text-lg text-gray-500 text-center mb-6"
            )
            ...
            with ui.card().classes("w-full p-6 bg-blue-50"):
                ...
                with ui.column().classes("gap-2"):
                    ui.label(
                        "P - Prime & Preview: Skim and prepare for deeper reading"
                    ).classes("text-sm")
                    ui.label(
                        "E - Engage & Explain: Deep reading and understanding in your own words"
                    ).classes("text-sm")
                    ui.label(
                        "C - Challenge & Connect: Critical thinking and making connections"
                    ).classes("text-sm")
                    ui.label(
                        "S - Solidify & Space: Create flashcards and spaced repetition review"
                    ).classes("text-sm")
    ...
    def _render_projects_grid(self, projects: list[Project]):
        ...
        ui.label(f"Your Projects ({len(projects)})").classes(
        ...
        with ui.card().classes("cursor-pointer hover:shadow-lg transition-all"):
            ...
            if cards_due > 0:
                with ui.row().classes("items-center gap-2 mb-3"):
                    ...
                    ui.label(f"{cards_due} cards due for review").classes(
            ...
            if total_sections > 0:
                ...
                ui.label(f"{int(progress * 100)}% Complete").classes(
                    "text-xs text-gray-500"
                )
    ...
    def _show_create_project_dialog(self):
        ...
        with ui.dialog() as dialog, ui.card().classes("w-full max-w-md mx-4"):
            ...
            project_name_input = (
                ui.input(
                    label="Project Name", placeholder="e.g., Learn Quantum Physics"
                )
                .classes("w-full")
                .props("autofocus")
            )
            ...
            async def create_project():
                ...
                if project:
                    ui.notify(
                        f'Project "{name}" created!', color="positive", position="top"
                    )
                    dialog.close()
    ...
    def _confirm_delete_project(self, project: Project):
        ...
        with ui.dialog() as dialog, ui.card().classes("max-w-sm mx-4"):
            ...
            ui.label(
                "This will delete all sections and flashcards. This action cannot be undone."
            ...
            async def delete_project():
                if self.db.delete_project(project.id):
                    ...
                    ui.notify(
                        "Failed to delete project", color="negative", position="top"
                    )

            with ui.row().classes("w-full gap-2 mt-4"):
                ui.button("Cancel", on_click=dialog.close).classes("flex-1 bg-gray-300")
                ui.button("Delete", on_click=delete_project).classes(
                    "flex-1 bg-red-500"
                )
