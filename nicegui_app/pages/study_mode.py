...
class StudyState:
    ...
    study_cards: List[Flashcard] = field(default_factory=list)
    study_mode: str = "due"  # 'due', 'all', or 'mastered'
...
class StudyModePage:
    ...
    def __init__(self, db: DatabaseRepository, project_id: int, user_id: int = None):
        ...
        # Encapsulated study session state
    ...
    def render(self):
        ...
        with ui.row().classes("w-full items-center gap-2 mb-6"):
            ...
            ui.label(f"Study: {self.project.name}").classes(
        ...
        def load_cards():
            ...
            if study_mode_radio.value == "Cards Due for Review":
                self.state.study_cards = self.db.get_flashcards_for_review(
                    self.project_id
                )
            elif study_mode_radio.value == "All Cards":
                self.state.study_cards = self.db.get_flashcards_by_project(
                    self.project_id
                )
            else:  # Mastered
                all_cards = self.db.get_flashcards_by_project(self.project_id)
                self.state.study_cards = [c for c in all_cards if c.is_mastered]

            # Shuffle if requested
            if shuffle_checkbox.value and self.state.study_cards:
                import random

                self.state.study_cards = random.sample(
                    self.state.study_cards, len(self.state.study_cards)
                )
        ...
        def render_study_session():
            ...
            with card_container:
                if not self.state.study_cards:
                    self._render_no_cards(study_mode_radio.value)
                    return

                # Progress
                progress = (
                    (self.state.current_index + 1) / len(self.state.study_cards)
                    if self.state.study_cards
                    else 0
                )
                ui.linear_progress(value=progress).classes("mb-2")
                ui.label(
                    f"Card {self.state.current_index + 1} of {len(self.state.study_cards)}"
                ).classes("text-sm text-gray-600 mb-4")

                # Current card
                current_card = self.state.study_cards[self.state.current_index]
    ...
    def _render_flashcard(self, card: Flashcard, refresh_callback):
        ...
        with ui.card().classes("w-full p-8 min-h-64"):
            ...
            if not self.state.show_answer:
                # Show answer button
                def reveal():
                    self.state.show_answer = True
                    refresh_callback()

                ui.button("Reveal Answer", icon="visibility", on_click=reveal).classes(
                    "bg-blue-500 text-lg px-6 py-3 mt-4"
                )
                ...
                with ui.row().classes("w-full gap-4 mt-6"):
                    ...
                    ui.button("Forgot", icon="close", on_click=review_forgot).classes(
                        "flex-1 bg-red-500 text-lg py-3"
                    )

                    ui.button("Knew It", icon="check", on_click=review_knew).classes(
                        "flex-1 bg-green-500 text-lg py-3"
                    )
    ...
    def _render_no_cards(self, mode: str):
        ...
        with ui.column().classes("items-center mt-12"):
            ...
            ui.label(f'No cards available for "{mode}"').classes(
                "text-xl text-gray-500 text-center mt-4"
            )
            ui.label("Create some flashcards in the Solidify phase").classes(
    ...
    def _review_card(self, card: Flashcard, knew_it: bool, refresh_callback):
        ...
        if self.state.current_index >= len(self.state.study_cards):
            ui.notify(
                "Study session complete! Great work!", color="positive", position="top"
            )
            self.state.current_index = 0
    ...
    def _render_stats_sidebar(self):
        ...
        with ui.card().classes("w-full max-w-md mx-auto mt-6 p-4"):
            ...
            with ui.row().classes("w-full justify-around"):
                ...
                with ui.column().classes("items-center"):
                    ui.label(str(stats.get("mastered_flashcards", 0))).classes(
                        "text-2xl font-bold text-green-600"
                    )
                    ui.label("Mastered").classes("text-xs text-gray-600")

                with ui.column().classes("items-center"):
                    ui.label(str(stats.get("cards_due_for_review", 0))).classes(
