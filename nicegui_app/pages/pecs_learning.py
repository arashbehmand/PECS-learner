...
class PECSLearningPage:
    ...
    def render(self):
        ...
        if not self.section:
            ...
            ui.button(
                "Back", on_click=lambda: ui.navigate.to(f"/project/{self.project_id}")
            ).classes("mt-4")
            return
        ...
        with ui.row().classes("w-full items-center gap-2 mb-4"):
            ...
            section_title = (
                self.section.title or f"Section {self.section.order_index + 1}"
            )
        ...
        with ui.card().classes("w-full mb-4 bg-gray-50"):
            ...
            with ui.scroll_area().classes("w-full h-64"):
                ui.markdown(self.section.content or "No content available").classes(
                    "text-sm"
                )
    ...
    def _render_completion_report(self):
        ...
        with ui.card().classes(
            "w-full p-6 bg-gradient-to-r from-green-50 to-blue-50 border-l-4 border-green-500"
        ):
            with ui.row().classes("w-full items-center gap-2 mb-4"):
                ...
                if completed_at:
                    ...
                    completed_date = datetime.fromisoformat(completed_at).strftime(
                        "%B %d, %Y at %I:%M %p"
                    )
                    ui.label(f"Completed: {completed_date}").classes(
                        "text-sm text-gray-600"
                    )

            with ui.expansion(
                "View your AI-generated learning summary", icon="insights"
            ).classes("w-full"):
                ...
                ui.button(
                    "Regenerate Summary", icon="refresh", on_click=regenerate
            ...
            if next_section:
                ...
                ui.label("Ready to continue your learning journey?").classes(
                ...
                with ui.row().classes("w-full gap-2"):
                    ui.button(
                        ...
                        on_click=lambda: ui.navigate.to(
                            f"/project/{self.project_id}/section/{next_section.id}"
                        ),
                ...
                ui.label("🎓 You've completed all sections in this project!").classes(
                    "text-lg font-semibold mt-4 mb-2 text-green-700"
                )
                ui.button(
                    "Back to Project",
                    icon="home",
    ...
    def _get_current_phase(self) -> int:
        ...
        if not pecs_data.get("prime_preview", {}).get("completed"):
            return 0
        if not pecs_data.get("engage_explain", {}).get("completed"):
            return 1
        if not pecs_data.get("challenge_connect", {}).get("completed"):
            return 2
        if not pecs_data.get("solidify_space", {}).get("completed"):
            return 3
    ...
    def _render_conversation_ui(
        self, phase_key: str, phase_data: dict, input_widget, is_completed: bool
    ):
            ...
            with ui.card().classes("w-full mt-4 bg-purple-50"):
                ...
                async def continue_conv():
                    await self._continue_conversation(
                        phase_key, config["feedback_type"], input_widget.value
                    )
        ...
        with ui.card().classes(card_classes):
            ...
            with ui.row().classes("w-full items-center gap-2 mb-4"):
                if is_completed:
                    ...
                    ui.icon("radio_button_unchecked", size="sm").classes(
                        "text-gray-400"
                    )
            ...
            if not is_unlocked:
                ui.label(f"🔒 Complete Phase {config['index']} first").classes(
            ...
            if is_completed:
                with ui.expansion("See your work", icon="visibility").classes("w-full"):
                    if phase_data.get(config["field"]):
                        ui.label(config["completed_label"]).classes(
                            "font-semibold mt-2"
                        )
                        ui.label(phase_data[config["field"]]).classes(
                            "text-sm p-2 bg-white rounded"
                        )
            ...
            # Active input area
                ...
                with ui.row().classes("w-full gap-2 mt-3"):
                    ui.button(
                        "Save",
                        icon="save",
                        on_click=lambda: self._save_phase_data(
                            phase_key, config["field"], input_widget.value
                        ),
                    ).classes("bg-blue-500")

                    ui.button(
                        "Get AI Feedback", icon="psychology", on_click=get_feedback
                    ).classes("bg-purple-500")

                    if phase_data.get(config["field"]):
                        ui.button(
                            "Mark Complete", icon="check", on_click=complete_phase
    ...
    def _render_solidify_phase(self):
        ...
        with ui.card().classes("w-full p-6 bg-gray-100"):
            ...
            with ui.row().classes("w-full items-center gap-2 mb-4"):
                ...
                ui.label("Phase 4: Solidify & Space").classes(
            ...
            ui.label(
                "Create Flashcards - Turn key concepts into spaced repetition cards"
            ).classes("text-sm text-gray-600 mb-4")

            # Flashcard management
            flashcards = self.db.get_flashcards_by_project(
                self.section.project_id, self.section_id
            )

            if flashcards:
                ui.label(f"Your Flashcards ({len(flashcards)})").classes(
                    "font-semibold mb-3"
                )
                for card in flashcards:
                    with ui.card().classes("w-full mb-2 bg-white"):
                        ui.label(f"Q: {card.question}").classes("text-sm font-semibold")
                        ui.label(f"A: {card.answer}").classes(
            ...
            question_input = ui.input(
                label="Question", placeholder="What question tests understanding?"
            ).classes("w-full")

            answer_input = (
                ui.textarea(label="Answer", placeholder="The answer...")
                .classes("w-full")
                .props("rows=3")
            )
    ...
    async def _get_ai_feedback(self, phase: str, feedback_type: str, user_input: str):
        ...
        if not self.llm_service.is_available():
            ui.notify(
                "AI features require an OpenAI API key. Set OPENAI_API_KEY environment variable.",
                color="warning",
                position="top",
            )
            return
        ...
        try:
            ...
            if feedback:
                ...
                ui.notify(
                    "Could not generate feedback", color="warning", position="top"
                )
    ...
    async def _complete_phase(self, phase: str):
        ...
        if phase == "solidify_space" and self.llm_service.is_available():
            ui.notify(
                "Generating your learning summary...", color="info", position="top"
            )
            await self._generate_completion_report()
        else:
    ...
    def _build_learning_context(
        self, include_flashcards=True, include_conversations=False
    ):
        """
        ...
        if "challenge_connect" in pecs_data:
            ...
            if include_conversations:
                context["conversations"]["challenge"] = challenge.get(
                    "ai_conversation", []
                )

        # Flashcards - ONLY committed ones from database (not ephemeral suggestions!)
        if include_flashcards:
            db_flashcards = self.db.get_flashcards_by_project(
                self.section.project_id, self.section_id
            )
            context["flashcards"] = [
                {"question": c.question, "answer": c.answer} for c in db_flashcards
            ]

        logger.debug(
            f"Built learning context: understanding={bool(context['understanding'])}, explanation={bool(context['explanation'])}, critical_thinking={bool(context['critical_thinking'])}, flashcards={len(context['flashcards'])}"
        )
    ...
    def _format_context_for_flashcards(self, context):
        ...
        formatted = "\n".join(parts)
        logger.info(
            f"Formatted flashcard context: {len(context.get('flashcards', []))} existing cards, {len(formatted)} chars total"
        )
    ...
    async def _generate_ai_flashcards(self, existing_suggestions=None):
        ...
        try:
            ...
            context = self._build_learning_context(
                include_flashcards=True, include_conversations=False
            )
            ...
            if suggestions:
                ...
                await self._show_flashcard_dialog(
                    suggestions, existing_suggestions or []
                )
            else:
    ...
    async def _show_flashcard_dialog(
        self, new_suggestions: list, previous_suggestions: list = None
    ):
        ...
        logger.info(
            f"Opening flashcard dialog with {len(all_suggestions)} total suggestions ({len(previous_suggestions)} previous + {len(new_suggestions)} new)"
        )
                ...
                with suggestions_container:
                    for i, suggestion in enumerate(all_suggestions):
                        ...
                        card_class = (
                            ...
                            if is_added
                            else "w-full mb-2 bg-purple-50"
                        )

                        with ui.card().classes(card_class):
                            ui.label(f"Card {i + 1}").classes("font-semibold mb-2")
                            ui.label(f"Q: {suggestion['question']}").classes(
                                "text-sm mb-1"
                            )
                            ui.label(f"A: {suggestion['answer']}").classes(
                                "text-sm mb-2"
                            )

                            if is_added:
                                ui.label("✓ Added to deck").classes(
                                ...
                                def make_add_handler(idx, sug):
                                    def handler():
                                        ...
                                        logger.info(
                                            f"Added flashcard {idx + 1} to database"
                                        )
                                        ui.notify(
                                            "Flashcard added!",
                                            color="positive",
            ...
            with button_row:
                ...
                async def generate_more():
                    ...
                    try:
                        ...
                        logger.info(
                            f"Generating more flashcards. Current DB has {len(context['flashcards'])} cards"
                        )
                        ...
                        if new_cards:
                            ...
                            ui.notify(
                                f"Generated {len(new_cards)} more cards!",
                                color="positive",
                                position="top",
                            )
                        else:
                        ...
                        logger.error(
                            f"Error generating more flashcards: {str(e)}", exc_info=True
                        )
                        ui.notify(f"Error: {str(e)}", color="negative", position="top")
                ...
                ui.button(
                    "Generate More", icon="add_circle", on_click=generate_more
                ).classes("bg-purple-500")
                ...
                ui.button("Done", icon="check", on_click=close_and_reload).classes(
                    "bg-green-500"
                )
    ...
    async def _continue_conversation(
        self, phase: str, _feedback_type: str, user_input: str
    ):
        """Continue an existing AI conversation with full context"""
        if not user_input:
            ui.notify(
                "Please write a question or response first",
                color="warning",
                position="top",
            )
            return
...
Be helpful and direct, not artificially enthusiastic."""
            ...
            feedback = await asyncio.to_thread(
                self.llm_service.make_llm_call, system_prompt, user_input
            )
            ...
            if feedback:
                ...
                ui.notify(
                    "Could not generate response", color="warning", position="top"
                )
    ...
    async def _generate_completion_report(self):
        ...
        context = self._build_learning_context(
            include_flashcards=True, include_conversations=True
        )
        ...
        if context.get("critical_thinking"):
            summary_parts.append("\n=== Challenge & Connect ===")
            summary_parts.append(
                f"Questions & connections: {context['critical_thinking']}"
            )
...
Be analytical and honest. Don't be overly enthusiastic or artificially encouraging. Write like a thoughtful reviewer, not a cheerleader."""
        ...
        logger.info(
            f"Generating completion report with {len(summary_text)} chars of context"
        )
        ...
        try:
            ...
            if report:
                ...
                self.db.update_section_pecs_data(
                    self.section_id, "solidify_space", solidify_data
                )
                logger.info(f"Completion report saved ({len(report)} chars)")

                # Reload to show the persistent card
                ui.notify(
                    "Learning summary generated!", color="positive", position="top"
                )
                ui.navigate.reload()
            else:
            ...
            dialog.close()
            ui.notify(
                f"Section completed! (Report generation failed: {str(e)})",
                color="positive",
                position="top",
            )
            ui.navigate.reload()
