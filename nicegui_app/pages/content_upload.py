...
class ContentUploadPage:
    ...
    def render(self):
        ...
        if existing_sections:
            with ui.card().classes('w-full bg-yellow-50 border border-yellow-200 mb-4'):
                with ui.row().classes('items-center gap-2'):
                    ...
                    ui.label(f'This project already has {len(existing_sections)} sections. New content will be appended.').classes('text-sm')
            ...
            with content_container:
                if paste_radio.value == 'Paste Text':
                    ...
                    ui.upload(
                        ...
                        auto_upload=True
                    ).classes('w-full').props('accept=".txt,.md,.epub,.pdf,.docx,.doc"')
        ...
        with ui.grid(columns=2).classes('w-full gap-4'):
            ...
            hierarchical_checkbox = ui.checkbox(
                'Enable hierarchical section detection',
                value=True
            )
            ui.label('Automatically detect chapters and headings').classes('text-sm text-gray-600')
        ...
        async def process_content():
            ...
            if not content:
                ui.notify('Please provide some text content to process', color='warning', position='top')
                return

            # Show progress
            with ui.dialog() as progress_dialog, ui.card():
                ui.label('Processing your material...').classes('text-lg font-bold')
                ui.label('Detecting sections and creating content structure').classes('text-sm text-gray-600 mt-2')
            ...
            try:
                ...
                if not sections:
                    ...
                    ui.notify('No valid sections could be created from the text', color='negative', position='top')
                    return
                ...
                if created_count > 0:
                    ui.notify(f'Successfully created {created_count} sections!', color='positive', position='top')
                    # Redirect to project view
                    ui.navigate.to(f'/project/{self.project_id}')
                else:
                    ui.notify('Failed to create sections', color='negative', position='top')
